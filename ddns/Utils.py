...
class Utils:
    ...
    #获取真实公网IP
    def getRealIP():
        ali = get_ip.Aliyunddns()
        return ali.CheckLocalip()
    ...
    def getRecordId(domain):
        ...
        request.add_query_param(\
            'DomainName', Utils.getConfigJson().get('First-level-domain'))
        #request.set_accept_format(\
            #Utils.getConfigJson().get('AccessKeyId'))
    ...
    def getConfigJson():
        cur_path = os.path.split(__file__)[0] + '/config.json'
